# Developer Guide: Sovereign Parts Compare
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/parts-compare.git
cd parts-compare
...
```bash
export SBB_SCRAPER_CONCURRENCY=your_value_here
export SBB_PROXY_LIST=your_value_here
export SBB_NLP_MODEL=your_value_here
export SBB_PRICE_ALERT_THRESHOLD=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-parts-compare)
...
```bash
# Build
docker build -t sovereign-parts-compare:dev .

# Run with env file
docker run --env-file .env -p 8783:8783 sovereign-parts-compare:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-parts-compare
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8783`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-parts-compare",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8783` in use | `lsof -i :8783` | Kill PID or change `SERVICE_PORT` |
